...
(
	...
	DataCreated SMALLDATETIME NULL,
...
(
	...
	(
		ID ASC
	) WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_PAGE_LOCKS = ON, ALLOW_ROW_LOCKS = ON) ON [PRIMARY]) ON [PRIMARY]
GO
...
INSERT dbo.Account
(
    AccountName, [Password], FullName, Email, Tell, DataCreated
)
VALUES
(   N'lgcong',                  -- AccountName - nvarchar(100)
    ...
    NULL,                  -- Tell - nvarchar(200)
    NULL -- DataCreated - smalldatetime
)
...
(   N'pttnga',                  -- AccountName - nvarchar(100)
    ...
    NULL,                  -- Tell - nvarchar(200)
    CAST(N'2020-06-04T00:00:00' AS SmallDateTime) -- DataCreated - smalldatetime
)
...
(   N'ttplinh',                  -- AccountName - nvarchar(100)
    ...
    N'lintttp@dlu.edu,vn',                  -- Email - nvarchar(1000)
    NULL,                  -- Tell - nvarchar(200)
    NULL -- DataCreated - smalldatetime
)
...
INSERT dbo.Bills
(
    ID, [Name], TableID, Amount, Discount, Tax, [Status], CheckoutDate, Account
)
VALUES
(   1,
    ...
    0.0,                   -- Tax - float
    1,                  -- Status - bit
    '2021-08-11', -- CheckoutDate - smalldatetime
...
AS
	...
	(   @Name, -- Name - nvarchar(1000)
	    @Unit, -- Unit - nvarchar(100)
	    @FoodCategoryID,   -- FoodCategoryID - int
	    @Price,   -- Price - int
	    @Notes  -- Notes - nvarchar(3000)
	    )
...
BEGIN 
	IF (NOT EXISTS
			(SELECT [Name] FROM dbo.Category WHERE @Name = [Name])
		)
...
begin
	Update Account 
	set AccountName = @AccountName, Password = @Password, FullName = @FullName, Email = @Email, Tell = @Tell, DataCreated = @DateCreated
